...
def create_report(request, pk):
    if request.user.is_labworker:
        ...
        if request.method == 'POST':
            ...
            try:
                send_mail(subject, plain_message, 'hospital_admin@gmail.com',  [patient_email], html_message=html_message, fail_silently=True)
...
def accept_doctor(request,pk):
    ...
    if(doctor_name is None):
        messages.error(request,'Not suffiecient information of doctor')
        return redirect('admin-dashboard')
    ...
    try:
        send_mail(subject, plain_message, 'hospital_admin@gmail.com',  [doctor_email], html_message=html_message, fail_silently=True)
    except BadHeaderError:
        return HttpResponse('Invalid header found')
...
def reject_doctor(request,pk):
    ...
    html_message = render_to_string('hospital_admin/reject-doctor-mail.html')
    plain_message = strip_tags(html_message)

    try:
        send_mail(subject, plain_message, 'ammu2734@gmail.com',  [doctor_email], html_message=html_message, fail_silently=True)
    except BadHeaderError:
        return HttpResponse('Invalid header found')
